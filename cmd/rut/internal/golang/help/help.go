...
Args:
	for i, arg := range args {
		...
		helpSuccess := "rut help"
		if i > 0 {
			helpSuccess += " " + strings.Join(args[:i], " ")
		}
		fmt.Fprintf(os.Stderr, "rut help %s: unknown help topic. Run '%s'.\n", strings.Join(args, " "), helpSuccess)
		base.SetExitStatus(2) // failed at 'rut help cmd'
		base.Exit()
	}
	...
	// not exit 2: succeeded at 'rut help cmd'.
}
